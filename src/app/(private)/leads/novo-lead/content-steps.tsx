...
// Http
import { registerLead } from '@/http/leads/register-lead'
  ...
  async function onSubmit(data: FormData) {
    ...
    try {
      ...
      const promisesDocuments = data.documents?.map((document) => {
        ...
        formData.append('students', leadId)
      ...
      if (promisesDocuments) {
        ...
        if (error) {
          toast.warning(
            'Ocorreu error no uploads de algum documento, acesse o pefil do lead para ver!',
            {
              duration: 3500,
              position: 'top-center',
              action: {
